...
# Fix 1: removed space after = (was: CUDA_VISIBLE_DEVICES= 0,1)
...
for seq_len in "${seq_len_list[@]}"; do
    ...
    for pred_len in "${pred_len_list[@]}"; do
        ...
        # Fix 2: replaced [run.py](http://run.py) hyperlink with plain run.py
        accelerate launch \
            ...
            --itr 1 \
            --des 'Exp' \
            --model_comment 'TimeLLM-household_1min' \

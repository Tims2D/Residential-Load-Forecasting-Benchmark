...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, path):
        ...
        if self.accelerator is not None:
            # Sync ALL ranks before touching the model
            self.accelerator.wait_for_everyone()
            # Unwrap DDP wrapper to get the raw model
            unwrapped_model = self.accelerator.unwrap_model(model)
            # Only rank 0 writes to disk — avoids file corruption and NCCL deadlock
            if self.accelerator.is_main_process:
                torch.save(unwrapped_model.state_dict(), path + '/' + 'checkpoint')
        else:
...
def test(args, accelerator, model, train_loader, vali_loader, criterion):
    ...
    with torch.no_grad():
        ...
        true = torch.from_numpy(
            np.array(torch.tensor(y, dtype=torch.float32))

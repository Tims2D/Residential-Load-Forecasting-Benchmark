...
class Model(nn.Module):
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        self.d_llm           = configs.llm_dim   # 4096 for LLaMA-7B
        ...
        try:
            ...
            print("Local LLaMA files not found. Attempting to download...")
            self.llm_model = LlamaModel.from_pretrained(
                'huggyllama/llama-7b',
                trust_remote_code=True,
        ...
        try:
            ...
            print("Local LLaMA tokenizer not found. Attempting to download...")
            self.tokenizer = LlamaTokenizer.from_pretrained(
                'huggyllama/llama-7b',
                trust_remote_code=True,
                local_files_only=False,
        ...
        self.vocab_size      = self.word_embeddings.shape[0]   # 32000 for LLaMA
        self.num_tokens      = 100
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # max_length=128 — LLaMA prompts are ~50-80 tokens
        prompt = self.tokenizer(
            prompt, return_tensors="pt", padding=True,
            truncation=True, max_length=128
        ).input_ids
        prompt_embeddings = self.llm_model.get_input_embeddings()(
            prompt.to(x_enc.device))   # [B*N, prompt_len, 4096]
        ...
        dec_out = self.llm_model(
            inputs_embeds=llm_enc_out.to(torch.float32)
        ).last_hidden_state

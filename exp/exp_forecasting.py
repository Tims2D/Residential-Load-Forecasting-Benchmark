...
class Exp_demand_Forecasting(Exp_Basic):
       ...
       # RAM_Usage meaurement
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.model in ['RNN', 'LSTM', 'GRU', 'BiLSTM', 'ResLSTM', 'Real_FITS', 'ModernTCN', 'SparseTSF','ConvLSTM']:
        ...
        print(Exp_demand_Forecasting.get_gpu_info())
        ...
        cpu_usage = psutil.cpu_percent(interval=1)  # measure over one second for a more accurate reading
    ...
    def test(self, setting, test=0):
        ...
        mae, mse, rmse, mape, mspe, rse, corr, r2, adj_r2 = metric(preds, trues, n_features=preds.shape[-1])
        print(f"MAE:{mae:.6f}, MSE:{mse:.6f}, RMSE:{rmse:.6f}, MAPE:{mape:.6f}, MSPE:{mspe:.6f}, RSE:{rse:.6f}, R2:{r2:.6f}, Adj_R2 {adj_r2:.6f}")
        f = open("result.txt", 'a')
        f.write(f"MAE:{mae:.6f}, MSE:{mse:.6f}, RMSE:{rmse:.6f}, MAPE:{mape:.6f}, MSPE:{mspe:.6f}, RSE:{rse:.6f}, R2:{r2:.6f}, Adj_R2:{adj_r2:.6f}")
        f.close()

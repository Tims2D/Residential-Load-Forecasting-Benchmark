...
from layers.Transformer_EncDec_our import iEncoder, iEncoderLayer, iFullAttention, iAttentionLayer
...
class Times2DBackbone(nn.Module):
    ...
    ############################################### Itrasnformer  ##########################################################
        ...
        self.encoder_itrnasformer = iEncoder(
            [
                iEncoderLayer(
                    iAttentionLayer(
                        iFullAttention(False, configs.factor, attention_dropout=configs.dropout,
                                      output_attention=configs.output_attention).to(self.device),
                    configs.d_model, configs.n_heads),
                    configs.d_model,
                    configs.d_ff,
                    dropout=configs.dropout,
                    activation=configs.activation).to(self.device)
                for l in range(configs.e_layers)
            ],
            norm_layer=torch.nn.LayerNorm(configs.d_model).to(self.device)
        )
        self.projector = nn.Linear(configs.d_model, configs.pred_len, bias=True).to(self.device)
...
################################################################################################################
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):    # x_enc [B, T, N]  x_mark_enc [B, T, N]
        ...
        enc_out = self.enc_embedding(x_enc, x_mark_enc) # covariates (e.g timestamp) can be also embedded as tokens [B , N, d_model]
        enc_out, attns = self.encoder_itrnasformer(enc_out, attn_mask=None)   #  [B , N, d_model]
      
        dec_out = self.projector(enc_out).permute(0, 2, 1)[:, :, :N] # [B, Pred_len, N]
        
         # De-Normalization from Non-stationary Transformer
        dec_out = dec_out * (stdev[:, 0, :].unsqueeze(1).repeat(1, self.pred_len, 1))
        dec_out = dec_out + (means[:, 0, :].unsqueeze(1).repeat(1, self.pred_len, 1))  # [B, Pred_len, N]
        ...
        combined  = dec_out  +z                                  # [B, N, Pred_len]

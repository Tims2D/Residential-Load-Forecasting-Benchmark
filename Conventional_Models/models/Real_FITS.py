...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.individual:
            ...
            self.freq_upsampler_real = nn.Linear(self.cut_freq, int(self.cut_freq*self.length_ratio)).to(self.device) # complex layer for frequency upcampling]
            self.freq_upsampler_imag = nn.Linear(self.cut_freq, int(self.cut_freq*self.length_ratio)).to(self.device) # complex layer for frequency upcampling]
    ...
    def forward(self, x):
        ...
        low_specxy_R = torch.zeros([low_specxy_real.size(0),int((self.seq_len+self.pred_len)/2+1), low_specxy_real.size(2)],
        ...
        out=(low_xy) * torch.sqrt(x_var) +x_mean    #[B, Seq+Pred, N]
        
        if self.task_name == 'Multivariate_forecasting':
            out = out[:, :, -1:]  # [B, ,Pred_len+ Seq_len, 1]
